...
function displayProductsByBrand(product) {
  ...
  filterProducts = productArray.find(function (temp) {
    return temp.id.substring(0, 2) === product.id;
  });
}
...
var filterProducts = productArray.slice(0);
...
function displayProducts() {
  const totalPages = Math.ceil(filterProducts.length / productsPerPage); // Tính tổng số trang
  ...
  const paginatedProducts = filterProducts.slice(startIndex, endIndex); // Lấy sản phẩm cho trang hiện tại
